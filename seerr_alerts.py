...
def run_approval_check():
    ...
    if not dm_enabled() or not discord_bot.dm_user_ids():
        ...
        return f"{total} awaiting approval. Discord DMs are off."

    if not fresh:
        return f"{total} awaiting approval, nothing new."
    ...
    message = f"{total} awaiting approval; announced {len(announced)} new ({delivered} DM(s) sent)"
    if failures:
